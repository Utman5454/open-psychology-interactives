...
(function () {
  ...
  /* Three respondents, in this order, rather than 200 to browse.

     121 and 18 differ by 0.02 s - the width of a rounding error - and went
     opposite ways. That pair is the whole argument, and it is not a freak: of
     the thirteen respondents scoring between 0.85 and 1.05 s, four shortlisted
     more Uplanders, five an even number and four fewer. 111 comes from the
     other end of the scale and also goes against the trend, so the lesson is
     not "high scores are unreliable" but "a score is not a person".

     The scatter, the correlation, the band table and the rule check are all
     still computed from the full 200: the group-level association is real and
     has to stay visible, or the tool teaches that the instrument measures
     nothing. */
  var FEATURED_IDS = [121, 18, 111];

  var EXP1_INITIAL = { step: 0, guess: null, revealed: false, made: 0, hits: 0, declined: 0 };
  var exp1 = null;

  function current() {
    var id = FEATURED_IDS[exp1.step];
  ...
  function renderRespondent() {
    ...
    respondentBox.appendChild(make("p", "respondent__id",
      "Respondent " + (exp1.step + 1) + " of " + FEATURED_IDS.length +
      " (number " + person.id + " in the sample of " + N + ")"));
  ...
  function renderTally() {
    if (!exp1.made && !exp1.declined) {
      tally.textContent =
        "No predictions yet. Across the full 200, guessing \"an even " +
        "shortlist\" every time would be right about " +
        Math.round(BASE_RATE * 100) + " per cent of the time without looking " +
        "at a single score.";
      return;
    }
    /* Three trials cannot establish an accuracy rate, and saying so is part of
       the teaching: the figure that carries the argument is the correlation
       over all 200, not the learner's score out of three. */
    tally.textContent =
      "You have committed to " + exp1.made + " prediction" +
      (exp1.made === 1 ? "" : "s") + ", been right " + exp1.hits +
      " and declined to guess " + exp1.declined + " time" +
      (exp1.declined === 1 ? "" : "s") + ". Three cases cannot establish an " +
      "accuracy rate - always answering \"an even shortlist\" would be right " +
      "about " + Math.round(BASE_RATE * 100) + " per cent of the time across " +
      "the full 200, and that is the number worth arguing with.";
  }
  ...
  function renderReveal() {
    ...
    /* The second respondent is the argument. Two scores 0.02 s apart, opposite
       outcomes - and the band count that shows the pair is ordinary. */
    if (exp1.step === 1) {
      var first = PEOPLE.filter(function (p) { return p.id === FEATURED_IDS[0]; })[0];
      var near = PEOPLE.filter(function (p) {
        return p.d >= 0.85 && p.d <= 1.05;
      });
      var split = { fewer: 0, even: 0, more: 0 };
      near.forEach(function (p) { split[bandOf(p.shortlist)] += 1; });
      var pair = make("div", "verdict");
      pair.setAttribute("data-tone", "warn");
      pair.appendChild(make("h5", "verdict__title",
        "Same score. Opposite shortlist."));
      pair.appendChild(make("p", "verdict__body",
        "Respondent " + first.id + " scored " + fixed(first.d) +
        " s and shortlisted " + first.shortlist + " of 4. This one scored " +
        fixed(person.d) + " s and shortlisted " + person.shortlist +
        ". The scores differ by " + fixed(Math.abs(person.d - first.d)) +
        " s and the behaviour differs by " +
        Math.abs(person.shortlist - first.shortlist) + " candidates."));
      pair.appendChild(make("p", "verdict__note",
        "Not a freak pairing: of the " + near.length + " respondents scoring " +
        "between 0.85 and 1.05 s, " + split.more + " shortlisted more " +
        "Uplanders, " + split.even + " an even number and " + split.fewer +
        " fewer. At any one score, the whole range of behaviour is still there."));
      revealBox.appendChild(pair);
    }

    /* The third closes it: the same failure from the other end of the scale. */
    if (exp1.step === FEATURED_IDS.length - 1) {
      var close = make("div", "verdict");
      close.setAttribute("data-tone", "good");
      close.appendChild(make("h5", "verdict__title",
        "The instrument is not the construct"));
      close.appendChild(make("p", "verdict__body",
        "Three respondents, and the score told you nothing usable about any of " +
        "them - including this one, at the low end. Across all 200 the " +
        "correlation is " + fixed(R) + ": real, and far too small to read " +
        "backwards onto a person. A group-level association and an individual " +
        "prediction are different claims."));
      close.appendChild(make("p", "verdict__note",
        "The scatter beside you is the honest picture. Experiment 2, below, " +
        "asks the same question of four other instruments."));
      revealBox.appendChild(close);
    }
  }
  ...
  function renderExp1(options) {
    ...
    if (nextButton) {
      nextButton.hidden =
        !exp1.revealed || exp1.step >= FEATURED_IDS.length - 1;
    }
  ...
  function goTo(step) {
    if (step >= FEATURED_IDS.length) { return; }
    exp1.step = step;
    exp1.guess = null;
    exp1.revealed = false;
    guessNote.textContent = "";
    renderExp1({
      announce: "Respondent " + (step + 1) + " of " + FEATURED_IDS.length +
        ", latency difference score " + fixed(current().d) +
        " seconds. Their shortlist is not shown yet."
    });
  }

  var nextButton = $('[data-action="next"]');
  nextButton.addEventListener("click", function () { goTo(exp1.step + 1); });

  $('[data-action="reveal"]').addEventListener("click", function () {
    if (exp1.revealed) {
      guessNote.textContent = exp1.step < FEATURED_IDS.length - 1
        ? "Already revealed. Move on to the next respondent."
        : "Already revealed. Experiment 2 is open below.";
    ...
    var last = exp1.step === FEATURED_IDS.length - 1;
    renderExp1({
      announce: "Respondent " + (exp1.step + 1) + " shortlisted " +
        current().shortlist + " of 4. " +
        (last ? "That is the third and last. Experiment 2 is now open below."
              : "Move on to the next respondent.")
    });
    if (last) { openExpTwo(); }
  });

  /* "Run twenty predictions for me" is gone. It sampled twenty respondents to
     show that a score-based rule underperforms the base rate - which the rule
     check under the scatter already states, computed over all two hundred and
     visible without pressing anything. A twenty-draw estimate of a figure the
     page can report exactly was the weaker of the two. */
  ...
  function openLabs() {
    ...
    latencyShell.announce(
      "Experiment 1 open, at the first of three respondents.",
      { immediate: true });
  }

  /* Experiment 2 opens once the three respondents are done, so the instruments
     are compared by somebody who has already watched one of them fail. */
  function openExpTwo() {
    if (!expTwo.hidden) { return; }
    expTwo.hidden = false;
    renderExp2();
    latencyShell.announce(
      "Experiment 2 is now open: five instruments, and what each one directly " +
      "observes.", { immediate: true });
  }
  ...
  latencyShell.announce(
    "Ready. Answer the question above to open Experiment 1.",
    { immediate: true });
})();
