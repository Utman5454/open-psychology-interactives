...
(function () {
  ...
  function renderTables(r) {
    ...
    dfNote.textContent =
      ...
      "for the normal distribution the t curve is approaching. The gap " +
        "is what it costs to estimate the standard deviation from the " +
        "same small sample that produced the means, and it closes as the " +
        "study grows. The interval above excludes zero exactly when the " +
        "test rejects. They are the same statement made twice, and the " +
        "interval is the more informative of the two because it names the " +
        "values the data are compatible with.";
  }
  ...
  function renderVerdict(r) {
    ...
    if (!lines.length) {
      lines.push(
        ...
        "before the data were seen. None of them can be checked from summary " +
        "statistics.");
    }
  ...
  var CONCLUSIONS = {
    ...
    trend: {
      ...
      text:
        "The authors chose α = .05 in advance and the result did not " +
          "reach it. \"Marginally significant\" and \"approaching " +
          "significance\" describe a failed test in language borrowed from " +
          "a passed one. There is no threshold at which a result becomes " +
          "marginal: .087 is not closer to .05 in any sense that matters. " +
          "If the threshold was worth setting, it is worth respecting; if " +
          "it was not, it should not have been set."
    },
    probably: {
      correct: false,
