...
(function () {
  ...
  function renderResultsProse(stats, meta) {
    ...
    if (simulated) {
      resultsBody.appendChild(make("p", null,
        "Generated by the model documented at the top of tool.js. The second " +
        "block has a deliberately smaller effect because two-thirds of its " +
        "trials are incongruent — that shrinkage is built into the generator, " +
        "not discovered by it, so the pair illustrates the proportion-" +
        "congruency finding rather than providing evidence for it. Run the " +
        "same two mixes yourself and compare."));
      resultsBody.appendChild(make("p", null,
        "Both seeds were chosen so that the sample means land near the " +
        "generator's parameters, because a worked example whose job is to " +
        "show a pattern should show it. That is worth knowing: even with " +
        "eighty trials per condition, plenty of other seeds produce a " +
        "facilitation of zero or below from exactly the same model."));
    } else {
      resultsBody.appendChild(make("p", null,
  ...
  function renderExcluded(stats) {
    ...
    excludedBody.appendChild(make("p", null,
      ...
      LAPSE_MS + " ms is a lapse of a different kind. Both are kept out of " +
      "the condition means and counted here instead. The cut-offs are a " +
      "documented choice, not a fact about the data — moving them would move " +
      "the numbers above, which is worth knowing before quoting any of them."));
    excludedBody.appendChild(make("p", null,
  ...
  var CHALLENGE_ITEMS = [
    {
      ...
      why: "Much smaller, usually close to nothing. Ink colour barely delays " +
        "reading a word, while a word substantially delays naming an ink. That " +
        "asymmetry is the observation the automaticity account was invented to " +
        "handle, and any account of the effect has to produce it."
    },
    {
      id: "proportion",
      text: "Two-thirds of the trials are incongruent instead of one-third.",
      answer: "smaller",
      why: "Smaller. When conflict is the norm, the system gives the word less " +
        "weight, and the difference between conditions shrinks. This is the " +
        "one row you can check on this page: run both mixes, or load the " +
        "worked example, where the shrinkage is built into the second " +
        "simulated block rather than discovered by it."
    },
    {
      id: "case",
    ...
    },
    {
      ...
      answer: "larger",
      why: "Larger. When the response is itself a spoken colour word, it " +
        "competes directly with the word on the screen, which is the same kind " +
        "of thing. Pressing a key puts one translation step between the two and " +
        "reduces the conflict. This is a reason to be careful comparing effect " +
        "sizes across studies that used different response modes."
    }
  ];
