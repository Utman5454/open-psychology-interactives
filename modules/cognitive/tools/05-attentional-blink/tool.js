...
(function () {
  ...
  function renderResultsProse(stats, meta) {
    ...
    if (simulated) {
      resultsBody.appendChild(make("p", null,
        "Generated by the model documented at the top of tool.js, with a fixed " +
        "seed, so it is identical every time. The lag-1 value is set above " +
        "lags 2 and 3 because lag-1 sparing is a robust finding — it is built " +
        "in here, not discovered, so this example illustrates the pattern " +
        "rather than providing evidence for it."));
    } else {
      resultsBody.appendChild(make("p", null,
        "Four trials per lag, in a browser tab, at a requested item duration " +
        "the browser cannot guarantee. A single lag point here moves by 25 " +
        "percentage points on one trial, so treat the shape as suggestive and " +
        "compare it with the worked example rather than with anybody else."));
    }
  ...
  var CHALLENGE_CLAIMS = [
    ...
    },
    {
      ...
      answer: "supported",
      why: "This is an argument about what the shape constrains, not a claim " +
        "about a mechanism, and the shape does constrain it. Note that in the " +
        "worked example lag-1 sparing is built into the generator, so it is " +
        "the published finding rather than this page's data that supports it."
    },
    {
      id: "capacity",
