...
(function () {
  ...
  function buildVerdict(theCase) {
    ...
    // Computed separately for each person rather than assumed equal, so a
    // future change to a case's facet scores cannot silently break the
    // "equal by construction" claim this and the paragraphs below make. If
    // the case data is ever edited so the two scores stop matching, none of
    // this prose may go on asserting they are equal.
    var scoreA = domainScore(theCase.people[0], BALANCED_MIX);
    var scoreB = domainScore(theCase.people[1], BALANCED_MIX);
    var scoresEqual = scoreA === scoreB;

    var lead =
      right === total
        ? "All " + total + " correct." +
          (scoresEqual
            ? " You separated two people whose broad " + theCase.domain +
              " scores are identical."
            : "")
        : right === 0
        ? "None matched." +
          (scoresEqual
            ? " Which is worth sitting with, because the broad score gave you " +
              "nothing to go on."
            : " With the domain scores no longer equal here, that is not the " +
              "intended demonstration — see below.")
    ...
    verdictBody.appendChild(
      make("p", null,
        ...
        ". " +
        (scoresEqual
          ? "Both average out to a " + theCase.domain + " score of " + fmt(scoreA) +
            ", so the broad score is blind to the difference by construction — " +
            "it is the average of the two things that differ."
          : theCase.people[0].name + " averages " + fmt(scoreA) + " and " +
            theCase.people[1].name + " averages " + fmt(scoreB) + " on " +
            theCase.domain + " — not the equal broad score this case is meant " +
            "to demonstrate."))
    );
    ...
    verdictBody.appendChild(
      make("p", null,
        scoresEqual
          ...
          : "The two domain-score columns are not identical here either, because " +
            "the case data above no longer produces the equal broad score this " +
            "demonstration is meant to rely on.")
    );
