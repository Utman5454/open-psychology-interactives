...
1. Predict whether a scale correlating with workplace effectiveness is
   thereby shown to measure something distinct from personality and
   reasoning.
2. Read the correlation matrix at the default settings and find the
   self-report emotional-intelligence measure's overlap with the
   personality questionnaire.
3. Add each emotional-intelligence measure in turn to a baseline of
   personality and reasoning, and read how much predicted variance in
   the outcome each one adds.
4. Work through the five marketing claims and see which the current
   settings support, partly support, or do not support.

**What to look for.** At the default settings, the self-report
emotional-intelligence measure correlates 0.63 with the ordinary
personality questionnaire, more than it correlates with the outcome it
is sold on predicting. Personality and reasoning alone already account
for about 13% of the outcome's variance; adding the self-report
measure raises that by about three points, and adding the
performance-based measure by about five, neither a large jump against
the label's promise.

**Debrief.** A measure correlates with a real-world outcome. What else
...
claims with debrief, allow 25 to 30 minutes.
