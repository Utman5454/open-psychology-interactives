...
2. Load "Strong general factor" and read the six uniform loadings and
   the proportion of variance the first factor captures.
3. Load "Broad group factors" instead and compare the loadings and
   proportion against the first preset.
4. Load "Weak task relations" and see what a first factor looks like
   when the tasks barely correlate at all.

**What to look for.** Under "Strong general factor," all six tasks
load at about 0.81 on the first factor, which captures about 66% of
