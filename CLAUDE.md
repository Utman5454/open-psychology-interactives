...
Before anything else, read `PROJECT_STATE.md`: it says what is working, the
last verified milestone, the current branch, the invariants, the known
defects and what is next. Then run `python3 scripts/check-all.py --quick` to
confirm the tree is sound. Update `PROJECT_STATE.md` in the same commit as any
milestone; record durable choices in `docs/decisions.md` and hard-won lessons
in `docs/lessons.md`.
